...
export function SavedParkingCard({
  ...
  const placementClasses = placement === 'sheet'
    ? 'relative w-full'
    : 'absolute bottom-[calc(5rem+env(safe-area-inset-bottom))] left-2 z-20 max-w-[calc(100%-1rem)] md:bottom-4 md:left-4 md:max-w-sm';
  return (
    <aside data-testid="saved-parking-card" className={`${placementClasses} rounded-2xl border p-3 shadow-xl ${t.headerBg} ${t.cardBorder}`}>
