...
export function SavedParkingCard({ plan, onRefresh, onReplace, onRemove, canEdit, t }) {
  ...
  return (
    <aside data-testid="saved-parking-card" className={`absolute bottom-[calc(5rem+env(safe-area-inset-bottom))] left-2 z-20 max-w-[calc(100%-1rem)] rounded-2xl border p-3 shadow-xl md:bottom-4 md:left-4 md:max-w-sm ${t.headerBg} ${t.cardBorder}`}>
      <p className={`text-[10px] font-black text-blue-600`}>已選停車場</p>
      ...
      <div className="mt-2 flex flex-wrap gap-1">
        <button type="button" onClick={onRefresh} className="min-h-11 rounded-xl bg-blue-600 px-2 text-[10px] font-black text-white">查看最新資訊</button>
        ...
        <button type="button" disabled={!canEdit} onClick={onRemove} className="min-h-11 rounded-xl border border-red-500 px-2 text-[10px] font-black text-red-500 disabled:opacity-50">移除</button>
      </div>
    </aside>
  );
}
